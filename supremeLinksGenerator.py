...
# Central
centralData = []
with open("LinksData/Central/Central.csv", 'r', encoding='utf-8') as f:
    centralData = f.read().split('\n')[1:]
    
for cd in centralData[:1]:
    data = cd.split(',')
    websiteName = data[0]
    websiteLink = data[1]
    newState = {}
...
# UPSC
# UPSCData = []
# with open("LinksData/Multiple/UPSC.csv",'r', encoding='utf-8') as f:   
...
# State
# states = os.listdir('LinksData/States')
...
#     stateOrganizations=[]
#     with open(f"LinksData/States/{state}",'r', encoding='utf-8') as f:
...
with open("documentLinksSupreme.json","w") as f:
